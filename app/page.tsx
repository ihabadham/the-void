...
import { Database, Calendar, Clock, TrendingUp, AlertCircle, CheckCircle, XCircle, Plus } from "lucide-react"
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const interviews = apps.filter((app: Application) => app.status === "interview").length
    const rejections = apps.filter((app: Application) => app.status === "rejected").length
    ...
    <div className="flex-1 space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <SidebarTrigger />
          <div>
            <h1 className="font-mono text-3xl font-medium text-white">Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="void-card">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 font-mono">Applications cast into the void</p>
          </CardContent>
        </Card>
        ...
        <Card className="void-card">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 font-mono">Awaiting response from the abyss</p>
          </CardContent>
        </Card>
        ...
        <Card className="void-card">
          ...
          <CardContent>
            <div className="text-2xl font-mono font-bold text-white">{stats.interviews}</div>
            <p className="text-xs text-gray-500 font-mono">Scheduled interrogations</p>
          </CardContent>
        </Card>

        <Card className="void-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-gray-400">Rejections</CardTitle>
            <XCircle className="h-4 w-4 text-red-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-mono font-bold text-white">{stats.rejections}</div>
            <p className="text-xs text-gray-500 font-mono">Returned to sender</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="void-card">
          ...
          <CardContent>
            {upcomingEvents.length === 0 ? (
              <p className="text-gray-500 font-mono text-sm">No upcoming events. The void is quiet.</p>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="text-[#00F57A] text-sm font-mono">
                          {new Date(app.nextDate!).toLocaleDateString()}
                        </p>
                        <p className="text-gray-500 text-xs">{app.nextEvent}</p>
                      </div>
                    </div>
                  )
                })}
        ...
        <Card className="void-card">
          <CardHeader>
            <CardTitle className="font-mono text-white flex items-center gap-2">
              <TrendingUp className="h-5 w-5 text-[#00F57A]" />
              Recent Applications
            </CardTitle>
          </CardHeader>
          <CardContent>
            {recentApplications.length === 0 ? (
              <p className="text-gray-500 font-mono text-sm">No applications logged. Begin the ritual.</p>
            ) : (
              <div className="space-y-3">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Badge className={`${statusColors[app.status]} text-black text-xs font-mono`}>
