...
import { FileText, Upload, Download, Trash2, Search, Eye, Plus } from "lucide-react"
...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    // Load documents and applications from localStorage
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newDocument: Document = {
      ...
      name: file.name,
      type:
        file.name.toLowerCase().includes("cv") || file.name.toLowerCase().includes("resume")
          ? "cv"
  ...
  const confirmDelete = () => {
    ...
    setDeleteModal({ isOpen: false, documentId: "", documentName: "", applicationName: "" })
  }
    ...
    <div className="flex-1 space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <SidebarTrigger />
          <div>
            <h1 className="font-mono text-3xl font-medium text-white">Documents</h1>
      ...
      {showUploadForm && (
        <Card className="void-card">
          <CardHeader>
            <CardTitle className="font-mono text-white">Add Document to Application</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <label className="text-gray-300 font-mono text-sm">Select Application *</label>
              <select
            ...
            {!selectedApplicationId && (
              <p className="text-gray-500 text-sm font-mono">Select an application first to attach the document.</p>
            )}
          </CardContent>
        </Card>
      )}
        ...
        <Card className="void-card border-yellow-700">
          <CardContent className="flex flex-col items-center justify-center py-8">
            <div className="text-center">
              ...
              <h3 className="font-mono text-lg text-white mb-2">No Applications Found</h3>
              <p className="text-yellow-400 font-mono text-sm mb-4">
                You need to log at least one application before you can attach documents.
              </p>
              <Button
        ...
        <Card className="void-card">
          <CardContent className="flex flex-col items-center justify-center py-12">
            <div className="text-center">
              ...
              <h3 className="font-mono text-lg text-white mb-2">No Matching Documents</h3>
              <p className="text-gray-500 font-mono text-sm">Try adjusting your search criteria.</p>
            </div>
          </CardContent>
        </Card>
      ) : (
        filteredDocuments.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredDocuments.map((doc) => {
              ...
              return (
                <Card key={doc.id} className="void-card hover:border-gray-600 transition-colors">
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3 flex-1">
                        <FileText className="h-8 w-8 text-[#00F57A]" />
                        <div className="flex-1 min-w-0">
                          <CardTitle className="font-mono text-white text-sm truncate">{doc.name}</CardTitle>
                          ...
                          <div className="mt-2 p-2 rounded border border-gray-700 bg-gray-900/50">
                            <p className="text-[#00F57A] text-xs font-mono">Attached to:</p>
                            <p className="text-white text-xs font-medium">{doc.applicationCompany}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <CardContent className="space-y-3">
                    <div className="text-sm text-gray-400 space-y-1">
                      <p className="font-mono">Size: {formatFileSize(doc.size)}</p>
                      <p className="font-mono">Uploaded: {new Date(doc.uploadDate).toLocaleDateString()}</p>
                    </div>
      ...
      {documents.length > 0 && (
        <Card className="void-card">
          <CardHeader>
            <CardTitle className="font-mono text-white">Document Statistics</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {Object.entries(documentTypes).map(([type, config]) => {
                ...
                return (
                  <div key={type} className="text-center p-4 rounded border border-gray-700">
                    <p className="text-2xl font-mono font-bold text-white">{count}</p>
                    <p className="text-gray-400 text-sm font-mono">{config.label}</p>
                  </div>
                )
              })}
            </div>
      ...
      <ConfirmationModal
        ...
        onClose={() => setDeleteModal({ isOpen: false, documentId: "", documentName: "", applicationName: "" })}
